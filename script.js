class WishlistApp {
    ...
    createFolder() {
        ...
        // Сохраняем папку отдельно по id и только после успешной записи добавляем в локальный массив
        this.db.ref('folders/' + folder.id).set(folder, (error) => {
            if (!error) {
                this.folders.push(folder);
                this.renderFolders();
                this.hideCreateFolderModal();
            } else {
                alert('Ошибка при сохранении папки: ' + error.message);
            }
        });
    }
